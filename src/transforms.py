...
class VVenC(object):
    ...
    def encoder(self, sample, image_path, get_cmd=False):
        ...
        enc_cmd  = f"ffmpeg -y -i {image_path}{self.in_format} -pix_fmt {self.ffmpeg_color_subsampling} {image_path}.yuv && "
    ...
    def decoder(self, image_path, sample_width, sample_height, get_cmd=False):
        ...
        dec_cmd += f"ffmpeg -y -f rawvideo -video_size {sample_width}x{sample_height} -pix_fmt {self.ffmpeg_color_subsampling} -i {image_path}_rec.yuv {image_path}{self.out_format} && "
...
class x265(object):
    ...
    def encoder(self, sample, image_path, get_cmd=False):
        sample_width, sample_height = sample.size
        enc_cmd  = f"ffmpeg -y -i {image_path}{self.in_format} -pix_fmt {self.ffmpeg_color_subsampling} {image_path}.yuv && "
        #"-c:v libx265 -preset slow -x265-params pools=\"none\":amp=1:rskip=0:no-limit-modes=1:no-info=1:fps=1:frame-threads=1:keyint=1:ref=1:no-open-gop=1:weightp=0:weightb=0:cutree=0:rc-lookahead=0:bframes=0:scenecut=0:b-adapt=0:repeat-headers=1:rdoq-level=2:psy-rdoq=0:psy-rd=0"
        enc_cmd += f"ffmpeg -y -f rawvideo -video_size {sample_width}x{sample_height} -i {image_path}.yuv \
    ...
    def decoder(self, image_path, sample_width, sample_height, get_cmd=False):
        #dec_cmd += " -c:v libx265 -x265-params pools=\"none\":frame-threads=1:bframes=0"
        dec_cmd  = f" ffmpeg -y -i {image_path}{self.bitstream_format} -f rawvideo -video_size {sample_width}x{sample_height} -pix_fmt {self.ffmpeg_color_subsampling} {image_path}_rec.yuv && "
        dec_cmd += "ffmpeg -y -f rawvideo -video_size {sample_width}x{sample_height} -pix_fmt {self.ffmpeg_color_subsampling} -i {image_path}_rec.yuv {image_path}{self.out_format} && "
        dec_cmd += f"rm {image_path}_rec.yuv"
